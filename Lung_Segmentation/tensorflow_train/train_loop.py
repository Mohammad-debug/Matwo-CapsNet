...
class MainLoopBase(object):
    ...
    def write_param(self):
            #load param
            excluded_keys = ['sess','coord','files_to_copy','additional_summaries_placeholders_val','config']
            net_param=dict((key, value) for (key, value) in self.__dict__.items() if key not in excluded_keys)
            #set output name
            files_name='net'
            if 'network' in net_param:
                file_name=net_param['network'].__name__
            #create document
            doc_path=path.join(self.output_folder+'/',file_name+".txt")
            f= open(doc_path,"w+")
            for key in sorted(net_param):
                if hasattr(net_param[key], '__call__'):
                    to_print=key+'='+str(net_param[key].__name__)
                elif key is 'optimizer':
                    to_print=key+'='+str(net_param[key].name)
                else:
                    to_print=key+'='+str(net_param[key])
